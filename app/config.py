...
# 2026-08-26(2차) — 시군 클릭 시 자동 생성되는 현장 대응 체크리스트(app/field_response.py)용.
# 정책 체크리스트(POLICY_DISCLAIMER 취지)와 뉴스 검색(오래되거나 소규모 발생은 보도가
# 없을 수 있다는 취지)을 한 화면에 같이 보여주므로 안내문도 하나로 합친다.
FIELD_RESPONSE_DISCLAIMER = (
    "이 체크리스트는 방역실시요령·긴급행동지침(SOP) 검색 결과를 이 시군 상황에 맞춰 "
    "요약한 참고용 정보이며, 법적 효력이 있는 유권해석이 아닙니다. 뉴스 요약은 실제 "
    "웹 검색 결과에 기반하지만 오래되었거나 소규모인 발생은 보도가 거의 없을 수 "
    "있습니다. 실제 방역 조치는 반드시 관할 가축방역기관에 재확인하십시오."
)
